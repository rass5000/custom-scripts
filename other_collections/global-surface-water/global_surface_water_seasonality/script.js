...
function evaluatePixel(sample) {
  if (sample.seasonality == 0) {
    //Not water
    return [1, 1, 1]; //Return White
  } else if (sample.seasonality > 12) {
    //No data
    return [0.8, 0.8, 0.8]; //Return Light gray
  } else if (sample.seasonality > 0 && sample.seasonality <= 12) {
    //1 - 12 month of water
    return [
      visualizer.process(sample.seasonality)[0],
      visualizer.process(sample.seasonality)[1],
      visualizer.process(sample.seasonality)[2],
