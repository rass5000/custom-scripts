...
function evaluatePixel(sample) {
  if (sample.occurrence == 0) {
    //Not water
    return [1, 1, 1]; //Return White
  } else if (sample.occurrence > 100) {
    //No data
    return [0.8, 0.8, 0.8]; //Return Light gray
  } else if (sample.occurrence > 0 && sample.occurrence <= 100) {
    //1 - 100% occurrence
    return [
      visualizer.process(sample.occurrence)[0],
      visualizer.process(sample.occurrence)[1],
      visualizer.process(sample.occurrence)[2],
