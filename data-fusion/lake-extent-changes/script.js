...
if (date1_date > Landsat89_start_date) {
  ...
  date1_date < Landsat45TM_end_date &&
  date2_date > Landsat89_start_date
) {
  var ds2bands = ["B02", "B03", "B04", "B06"];
...
function evaluatePixel(p) {
  ...
  var b_1 = s1[ds1bands[0]],
    g_1 = s1[ds1bands[1]],
    r_1 = s1[ds1bands[2]],
    swir1_1 = s1[ds1bands[3]];
  var b_2 = s2[ds2bands[0]],
    g_2 = s2[ds2bands[1]],
    r_2 = s2[ds2bands[2]],
    swir1_2 = s2[ds2bands[3]];
  ...
  if (water_diff == 1) return [1, 0, 0];
  if (water_diff == -1) return [0, 0, 1];
  else if (water1 == 1) return [0.44, 0.54, 1];
  else return RGB;
}
