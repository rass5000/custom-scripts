...
function setup() {
  return {
    input: [
      ...
      { datasource: "l2a_t1", bands: ["B03", "B04", "B08", "B11", "B12"] },
...
function evaluatePixel(
  ...
  if (NDWI > 0.15 || NDVI > 0.35 || INDEX > 0.2) {
    ...
    if ((s1_2.VH < thr_VH) & (VH_diff < thr_VH_diff)) {
      // are classified as burned in S1 image from t2 via threshholds for VH backscatter and the calculated difference layer
