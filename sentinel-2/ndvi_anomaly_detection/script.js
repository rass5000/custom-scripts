...
function preProcessScenes(collections) {
  ...
  collections.scenes.orbits = collections.scenes.orbits.filter(function (
    ...
    return (
      orbitDateFrom.getMonth() === collections.to.getMonth() &&
      orbitDateFrom.getFullYear() >= collections.to.getFullYear() - nbPastYears
    );
  });
  return collections;
}
...
function calculateLoss(
  currentIndexAverage,
  pastIndexAverage,
  minimumAverageValuePremium,
  lowerTriggerPremium,
  higherTriggerPremium,
  minimumPayoutPremium,
  defaultValue
) {
  if (currentIndexAverage === null || pastIndexAverage === null)
    return defaultValue;
  if (
    pastIndexAverage < minimumAverageValuePremium ||
    pastIndexAverage <= currentIndexAverage
  )
    return 0;
  ...
  return Math.max(
    minimumPayoutPremium,
    (percentage - (lowerTriggerPremium - minimumPayoutPremium)) /
      higherTriggerPremium
  );
}

var calculateNdviAnomaly = function calculateNdviAnomaly(
  indexesAverages,
  pixelEvalMaxValue,
  defaultValue
) {
  if (indexesAverages.current === null || indexesAverages.past === null)
    return defaultValue;
  return Math.max(
...
const evaluatePixel = function (samples, scenes) {
  ...
  return valueInterpolate(
    calculateNdviAnomaly(
      indexesAverages,
      config2.ndviAnomaly.pixelEvalMaxValue,
      config2["default"].defaultOutputValue
    ),
    [
      config2["default"].defaultOutputValue,
      0 - config2.ndviAnomaly.pixelEvalMaxValue,
      0,
      config2.ndviAnomaly.pixelEvalMaxValue,
    ],
    [
...
var calculateIndexesForSamples = function calculateIndexesForSamples(
  samples,
  scenes,
  config,
  processSampleMethod
) {
  if (samples.length !== scenes.length)
    throw new Error("samples and scenes arrays do not have same length");
...
var calculatePastIndexesAverage = function calculatePastIndexesAverage(
  indexes,
  currentYear,
  config
) {
  ...
  return pastIndexes.count >= config.pastIndexesMinValuesNumber
    ? pastIndexes.sum / pastIndexes.count
    : null;
};
...
function calculateIndexAverages(samples, scenes, config, processSampleMethod) {
  ...
  var indexes = calculateIndexesForSamples(
    samples,
    scenes,
    config,
    processSampleMethod
  ...
  return {
    current:
      (currentYearIndex &&
        currentYearIndex.count >= config.currentIndexesMinValuesNumber &&
        currentYearIndex.sum / currentYearIndex.count) ||
      null,
