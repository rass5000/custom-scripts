...
function rgb2lab(rgb) {
  ...
  */
  ...
  function to_linear(val) {
    if (val > 0.04045) return Math.pow((val + 0.055) / 1.055, 2.4);
  ...
  function std_prep(val) {
    ...
    else return val * 7.787 + 16.0 / 116.0;
  }
...
function lab2rgb(Lab) {
  ...
  */
  ...
  function undo_std_prep(val) {
    if (Math.pow(val, 3.0) > 0.008856) return Math.pow(val, 3.0);
    else return (val - 16.0 / 116.0) / 7.787;
  }
  ...
  let var_b = x * 0.0557 + y * -0.204 + z * 1.057;

  function undo_linear(val) {
    if (val > 0.0031308) return 1.055 * Math.pow(val, 1.0 / 2.4) - 0.055;
    else return val * 12.92;
  }
...
class Colormap {
  ...
  */
  constructor(color_anchors, data_anchors, remap = true, uniform = false) {
    if (color_anchors.length < 1) throw "ColorMap requires at least one color.";
  ...
  get_color(data_value) {
    ...
    return lab2rgb(
      colorBlend(data_value, this.data_anchors, this.color_anchors)
    );
  }
}
