...
function evaluatePixel(samples) {
  ...
  let index0 =
    14.039 +
    (86.11 * (samples.B05 - samples.B04)) / (samples.B05 + samples.B04) +
    194.325 *
      Math.pow((samples.B05 - samples.B04) / (samples.B05 + samples.B04), 2);
  ...
  let viz1 = colorBlend(index1, scaleLimits, colorScale); // indexNumber = 2 | Chlorophyll a  low values < 5 mg / m3 (Soria-Perpinyà 2021)
  let index2 = Math.pow(
    10,
    -2.4792 * Math.log10(Math.max(samples.B03, samples.B02) / samples.B03) -
      0.0389
  );
  let viz2 = colorBlend(index2, scaleLimits, colorScale); // indexNumber = 3 | Cyanobacteria (phycocyanin) cells/mL (Potes et al. 2018)
  let index3 =
    115530.31 * Math.pow((samples.B03 * samples.B04) / samples.B02, 2.38);
  ...
  if (NDWI < -0) {
    // If NDWI is lower than 0 is not water, so return  true color
    imgVals = [...TrueColor, samples.dataMask];
  } else {
    // Evaluate indexNumber
    switch (indexNumber) {
  ...
  } // The library for tiffs works well only if there is only one channel returned. // So we encode the "no data" as NaN here and ignore NaNs on frontend.
