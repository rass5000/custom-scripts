...
function getColors(type, index, min, max, isSentinel3Flh) {
  ...
  switch (type) {
    ...
    case "tss":
      ...
      colors = colorBlend(index, [min, max], palette);
      break;
    default:
      break;
  }
 ...
 */
function isWater(
  availableWatermaskIndices,
  selectedWatermaskIndices,
  waterMax,
  cloudMax,
  isSentinel3
) {
...
function getForeground(foreground, backgroundLayer, naturalIndex, opacity) {
  ...
  return opacity === 1
    ? layer
    : blend(layer, backgroundLayer, alpha, 100 - alpha);
}
...
function getValue(params) {
  ...
  const backgroundLayer = getBackground(
    background,
    indices.natural,
