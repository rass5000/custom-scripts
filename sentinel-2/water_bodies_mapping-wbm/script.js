...
function wbi(r, g, b, nir, swir1, swir2) {
  ...
  try {
    ...
    if (filter_SSI && ws == 1) {
      //SHADOWS[3]
      if (aweish <= 0.1112 && ndvi > -0.2) {
        ws = 0;
      }
      if (aweinsh < 0.5 && ndvi > -0.2) {
        ws = 0;
      } //or 0.1897
      if (aweinsh < 0 || aweish <= 0 || ndvi > -0.1) {
        ws = 0;
      }
      //SNOW AREAS[6][7][8]
      if (
...
function preProcessScenes(collections) {
  collections.scenes.orbits = collections.scenes.orbits.filter(function (
    ...
    return (
      new Date(orbit.dateFrom) < new Date(toD) &&
...
function evaluatePixel(p) {
  nirDS = "B08";
  swir1DS = "B11";
  swir2DS = "B12"; // Bands for Sentinel-2
  ...
  if (waterAvg >= 0.1) return [0.44, 0.54, 1];
  else return RGB;
}
