...
function evaluatePixel(sample) {
  for (let i = 0; i < sample.length; i++) {
    ...
    if (sample[i].viewAzimuthMean > 1) {
      ...
      return bands.map(
        (band) => lum * calc_nbar(available[band], f_values[band], kernels)
      );
    }
  }
  return [0, 0, 0];
...
var f_values = {
  ...
  B03: [0.1306, 0.0178, 0.058],
  B04: [0.169, 0.0227, 0.0574],
  B08: [0.3093, 0.033, 0.1535],
  B11: [0.343, 0.0453, 0.1154],
...
function calc_kgeo(c) {
  ...
  //c 42 Lucht
  let Dsq =
    Math.pow(c.tan.sza, 2) +
    Math.pow(c.tan.vza, 2) -
    2 * c.tan.sza * c.tan.vza * c.cos.phi;
  ...
  // c 40 Lucht
  let big_o =
    (1 / Math.PI) * (t - Math.sin(t) * Math.cos(t)) * (c.sec.sza + c.sec.vza);
  ...
  let kgeo =
    ...
    c.sec.sza -
    c.sec.vza +
    (1 / 2) * (1 + c.cos_xi) * c.sec.sza * c.sec.vza;
...
function calc_kvol(c) {
  ...
  let kvol =
    ((Math.PI / 2 - xi) * c.cos_xi + Math.sin(xi)) / (c.cos.sza + c.cos.vza) -
    Math.PI / 4;
...
function calc_c_lambda(kernels, f) {
  ...
  return (
    calc_rho_modis(kernels.kgeo_vza_zero, kernels.kvol_vza_zero, f) /
    calc_rho_modis(kernels.kgeo, kernels.kvol, f)
  );
}
