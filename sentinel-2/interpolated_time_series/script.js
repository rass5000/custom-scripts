...
var sampled_dates = splitDateIntoEqualIntervals(
  start_date,
  end_date,
  36,
  1000 * 60 * 60 * 24
...
for (var i = 0; i < output_bands.length; i++) {
  for (var j = 0; j < bands_array_n; j++) {
    ...
    var sample_type = col_name.includes(qmask)
      ? SampleType.UINT8
      : SampleType.UINT16;
    output_setup.push({
      id: col_name,
      bands: bands_array_size,
...
function lininterp(x_arr, xp_arr, fp_arr) {
  ...
  for (var i = 0; i < x_arr.length; i++) {
    ...
    var interp_val = linearInterpolation(
      x,
      xp_arr[interval],
      fp_arr[interval],
      xp_arr[interval + 1],
...
function splitDateIntoEqualIntervals(
  startDate,
  endDate,
  numberOfIntervals,
  roundCoefficient
  ...
  for (let i = 0; i < numberOfIntervals; i++) {
    ...
    ndate = new Date(
      Math.round(ndate.getTime() / roundCoefficient) * roundCoefficient
    );
...
function evaluatePixel(samples, scenes) {
  ...
  var clp_thr = 0.3;
  while (
    valid_indices.length < 2 ||
    valid_dates[0] > sampled_dates[0] ||
    valid_dates[valid_dates.length - 1] <
      sampled_dates[sampled_dates.length - 1]
  ) {
    valid_dates = [];
    valid_indices = [];
    for (var i = 0; i < samples.length; i++) {
  ...
  for (var i = 0; i < output_bands.length - 1; i++) {
    var band = output_bands[i];
    var [band_data, mask_data] = lininterp(
      sampled_dates,
      valid_dates,
      valid_samples[band]
    );
  ...
  if (always_no_data) {
    for (var i = 0; i < output_bands.length - 1; i++) {
      var band = output_bands[i];
      interpolated_output[band] = new Array(
        interpolated_output[band].length
      ).fill(default_band_values["no_data"]);
    }
    interpolated_output[qmask] = new Array(
      interpolated_output[qmask].length
    ).fill(default_mask_values["no_data"]);
  }
