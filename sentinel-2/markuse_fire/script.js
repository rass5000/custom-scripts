...
function layerBlend(lay1, lay2, lay3, op1, op2, op3) {
  return lay1.map(function (num, index) {
    return (
      (num / 100) * op1 + (lay2[index] / 100) * op2 + (lay3[index] / 100) * op3
    );
  });
}
...
function evaluatePixel(sample) {
  ...
  const manualCorrection = [0.0, 0.0, 0.0];
  ...
  if (hotspot) {
    if (
      !cloudAvoidance ||
      (sample.CLP < cloudAvoidanceThreshold && sample.B02 < avoidanceHelper)
    ) {
      switch (style) {
        case 1:
          if (sample.B12 + sample.B11 > hsThreshold[0] / hsSensitivity)
            return [
          ...
          if (sample.B12 + sample.B11 > hsThreshold[1] / hsSensitivity)
            return [
              boost * 0.5 * sample.B12 + Viz[0],
          ...
          if (sample.B12 + sample.B11 > hsThreshold[2] / hsSensitivity)
            return [
              boost * 0.5 * sample.B12 + Viz[0],
          ...
          if (sample.B12 + sample.B11 > hsThreshold[3] / hsSensitivity)
            return [
              boost * 0.5 * sample.B12 + Viz[0],
        ...
        case 4:
          if (sample.B12 + sample.B11 > hsThreshold[3] / hsSensitivity)
            return [Viz[0] + 0.2, Viz[1] - 0.2, Viz[2] - 0.2, sample.dataMask];
          break;
        default:
      }
    }
